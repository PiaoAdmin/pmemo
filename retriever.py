...
class Retriever:
    def __init__(self, 
                 ...
                 key_term_memory: Optional[KeyTermMemory] = None,
        ...
        self.key_term_memory = key_term_memory
        # self.client = client 
    ...
    def _retrieve_key_memory(self, user_query, top_k_key, key_threshold):
        if not self.key_term_memory:
            return []
        print("Retriever: Searching key memory...")
        results = self.key_term_memory.search_key_memory(user_query, top_k=top_k_key, threshold=key_threshold)
        print(f"Retriever: Key memory recalled {len(results)} items.")
        return results

    def _retrieve_long_term_summary(self, user_query, top_k_ltm, ltm_threshold):
        print("Retriever: Searching long-term summaries...")
        results = self.user_long_term_memory.search_long_term_summary(user_query, top_k=top_k_ltm, threshold=ltm_threshold)
        print(f"Retriever: Long-term summary recalled {len(results)} items.")
        return results
    ...
    def retrieve_context(self, user_query: str, 
                         ...
                         retrieved_short_term=None
                         ):
        print(f"Retriever: Starting PARALLEL retrieval for query: '{user_query[:50]}...'")
        
        # 并行执行五个检索任务
        tasks = [
            lambda: self._retrieve_key_memory(user_query, top_k_key, key_threshold),
            lambda: self._retrieve_long_term_summary(user_query, top_k_ltm, ltm_threshold),
        ...
        retrieved_key_memory, retrieved_long_term, retrieved_mid_term_pages, retrieved_user_knowledge, retrieved_assistant_knowledge = results

        return {
            "retrieved_key_memory": retrieved_key_memory or [],
            "retrieved_long_term": retrieved_long_term or [],
            "retrieved_pages": retrieved_mid_term_pages or [], # List of page dicts
            "retrieved_short_term": retrieved_short_term or [],
